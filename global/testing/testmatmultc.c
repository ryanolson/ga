...
int main(int argc, char **argv)
{
    ...
    MP_INIT(argc,argv);
    if (!MA_init(MT_DBL,1,2000000)) {
        GA_Error("failed: ma_init(MT_DBL,1,2000000)",10);
    }
    GA_INIT(argc,argv);
    me = GA_Nodeid();

    /* broadcast array dimension to all other procs because MPI standard does
     * not guarantee command-line arguments are passed to all procs */
    if (0 == me) {
        if (argc >= 2) {
            errno = 0; /* to distinguish success/failure after call */
            user_size = strtol(argv[1], NULL, 10);
            /* check for various possible errors */
            if (errno == ERANGE || (errno != 0 && user_size == 0)) {
                perror("strtol");
                GA_Error("need a valid array dimension", EXIT_FAILURE);
            }
        }
    }
    /* the broadcast happens regardless */
    GA_Brdcst(&user_size, sizeof(long), 0);
    nums_m[0] = user_size;
    nums_n[0] = user_size;
    nums_k[0] = user_size;
