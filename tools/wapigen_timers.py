...
'''
    for name in sorted(functions):
        the_code += '''
        MPI_Reduce(&count_%s, &total_count, 1, MPI_LONG, MPI_SUM, 0, MPI_COMM_WORLD);
        MPI_Reduce(&time_%s, &total_time, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
        if (me == 0 && total_count > 0) {
            printf("%s,%%ld,%%lf,%%ld,%%lf,%%lf\\n",
                    count_%s, time_%s, total_count, total_time,
