...
void test_fetch_add()
{
  ...
  gop_times[me] = times;
  armci_msg_igop(gop_val, nproc, "+");
  armci_msg_igop(gop_times, nproc, "+");
  if (0 == me) {
    for (i = 0; i < nproc; i++) {
...
void test_fetch_add_long()
{
  ...
  gop_times[me] = times;
  armci_msg_lgop(gop_val, nproc, "+");
  armci_msg_lgop(gop_times, nproc, "+");
  if (0 == me) {
    for (i = 0; i < nproc; i++) {
      printf("process %d incremented the counter %d times value=%d\n",
              i, gop_times[i], gop_val[i]);
...
void test_swap_long()
{
  ...
  for (i = 0; i < LOOP; i++) {
    val = LOCKED;
    do {
      rc = ARMCI_Rmw(ARMCI_SWAP_LONG, &val, arr[0], whatever, 0);
      assert(rc == 0);
    }
    while (val == LOCKED);
    val++;
